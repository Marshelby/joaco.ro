-- Las cantidades del contrato JSON son números, no texto libre.
...
begin
  ...
  where c.id = p_cliente_id and c.activo = true;
  ...
  select * into v_pedido_existente
  ...
  if found then
    ...
    return query select v_pedido_existente.id, v_pedido_existente.numero_pedido,
      v_pedido_existente.estado, v_pedido_existente.subtotal,
      v_pedido_existente.costo_entrega, v_pedido_existente.descuento,
  ...
  insert into public.pedidos (
    cliente_id, canal_origen, estado,
    nombre_cliente_snapshot, telefono_cliente_snapshot, email_cliente_snapshot,
    direccion_cliente_id, direccion_snapshot, comuna_snapshot, region_snapshot,
    referencia_direccion_snapshot, subtotal, costo_entrega, descuento, total,
    observacion_general, fecha_entrega, clave_idempotencia
  ) values (
    p_cliente_id, 'web', 'recibido',
    v_cliente.nombre, v_cliente.telefono, v_cliente.email,
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    if v_item -> 'cantidad' is null or jsonb_typeof(v_item -> 'cantidad') <> 'number' then
    ...
    select pp.id, pp.producto_id, pp.nombre as nombre_presentacion,
      pp.cantidad as cantidad_presentacion, pp.unidad, pp.precio_neto,
      pp.precio_final, pr.nombre as nombre_producto
    into v_presentacion
    ...
    insert into public.items_pedido (
      pedido_id, producto_id, presentacion_producto_id,
      nombre_producto_snapshot, nombre_presentacion_snapshot, unidad_snapshot,
      cantidad, precio_neto_unitario_snapshot,
      precio_final_unitario_snapshot, total_linea, observacion_cliente
    ) values (
      v_pedido.id, v_presentacion.producto_id, v_presentacion.id,
      v_presentacion.nombre_producto, v_presentacion.nombre_presentacion,
      v_presentacion.unidad, v_cantidad, v_presentacion.precio_neto,
      v_presentacion.precio_final,
      round(v_cantidad * v_presentacion.precio_final)::bigint, null
    );
  end loop;
  ...
  set subtotal = v_subtotal, costo_entrega = 0, descuento = 0, total = v_subtotal
  where id = v_pedido.id
  returning * into v_pedido;

  return query select v_pedido.id, v_pedido.numero_pedido, v_pedido.estado,
    v_pedido.subtotal, v_pedido.costo_entrega, v_pedido.descuento, v_pedido.total;
end;
$$;
