...
export default async function AdminOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4"><OrderStatusBadge estado={pedido.estado} /><div className="flex flex-wrap items-start gap-3">{pedido.estado === "recibido" ? <OrderAcceptAction pedidoId={pedido.id} /> : null}{["recibido", "en_revision", "confirmado", "programado", "preparando"].includes(pedido.estado) ? <OrderCancelAction pedidoId={pedido.id} /> : null}</div></div>
