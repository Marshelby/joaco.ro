-- C5: pgcrypto se instala en el esquema extensions en Supabase; no depende del search_path público.

create or replace function public.crear_invitacion_acceso_cliente_administrativa(
  p_cliente_id uuid,
  p_correo_destino text
)
returns table (invitacion_id uuid, token text, fecha_expiracion timestamptz)
language plpgsql
security definer
...
begin
  ...
  if p_cliente_id is null or v_correo_destino !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' then raise exception 'CORREO_INVALIDO'; end if;
  select * into v_cliente from public.clientes c where c.id = p_cliente_id for update;
  if not found then raise exception 'CLIENTE_NO_ENCONTRADO'; end if;
  if not v_cliente.activo then raise exception 'CLIENTE_INACTIVO'; end if;
  if v_cliente.usuario_id is not null then raise exception 'CLIENTE_YA_VINCULADO'; end if;
  update public.invitaciones_acceso_cliente i set estado = case when i.fecha_expiracion <= now() then 'expirada' else 'revocada' end where i.cliente_id = p_cliente_id and i.estado = 'pendiente';
  v_token := encode(extensions.gen_random_bytes(32), 'hex');
  v_token_hash := encode(extensions.digest(v_token, 'sha256'), 'hex');
  insert into public.invitaciones_acceso_cliente (cliente_id, correo_destino, token_hash, estado, creada_por, fecha_expiracion)
  values (p_cliente_id, v_correo_destino, v_token_hash, 'pendiente', auth.uid(), v_fecha_expiracion)
...
begin
  ...
  if found and v_cliente_actual.id <> v_cliente_b2b.id then
    ...
    update public.clientes set usuario_id = null where id = v_cliente_actual.id;
    v_cliente_previo_id := v_cliente_actual.id;
  end if;
  update public.clientes set usuario_id = v_usuario_id where id = v_cliente_b2b.id;
  update public.invitaciones_acceso_cliente set estado = 'revocada' where cliente_id = v_cliente_b2b.id and estado = 'pendiente' and id <> v_invitacion.id;
  update public.invitaciones_acceso_cliente set estado = 'aceptada', fecha_aceptacion = now(), aceptada_por = v_usuario_id, cliente_previo_id = v_cliente_previo_id where id = v_invitacion.id;
