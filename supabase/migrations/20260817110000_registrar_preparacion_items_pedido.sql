-- P2: registra sólo excepciones de preparación. La finalización y el cálculo
-- financiero permanecen fuera de este contrato.

create function public.guardar_preparacion_item_pedido_administrativo(
...
begin
  ...
  if p_item_pedido_id is null then
    raise exception 'ITEM_PEDIDO_NO_ENCONTRADO';
  end if;

  if p_cantidad_preparada is null then
    raise exception 'CANTIDAD_PREPARADA_REQUERIDA';
  end if;

  if p_cantidad_preparada < 0 then
    raise exception 'CANTIDAD_PREPARADA_INVALIDA';
  end if;
  ...
  if not found or v_item.pedido_id is distinct from v_pedido.id then
  ...
  if p_cantidad_preparada > v_item.cantidad then
    raise exception 'CANTIDAD_PREPARADA_SUPERA_SOLICITADA';
  end if;

  v_modo := coalesce(
    v_item.modo_cantidad_snapshot,
    case
      when upper(btrim(v_item.unidad_snapshot)) = 'KG'
        and lower(coalesce(v_item.nombre_presentacion_snapshot, '')) !~ '(saco|malla|caja|paquete|docena)'
        then 'kg_fraccionable'
      when lower(coalesce(v_item.nombre_presentacion_snapshot, '')) ~ '(saco|malla|caja|paquete|docena)'
        then 'presentacion_cerrada'
      else 'unidad'
    end
  );

  if v_modo in ('presentacion_cerrada', 'unidad')
    and p_cantidad_preparada <> trunc(p_cantidad_preparada) then
    raise exception 'CANTIDAD_PREPARADA_DEBE_SER_ENTERA';
  end if;

  update public.pedidos
  set preparacion_estado = 'pendiente'
  where id = v_pedido.id
    and preparacion_estado is null;

  if p_cantidad_preparada = v_item.cantidad then
    delete from public.preparacion_items_pedido
    where preparacion_items_pedido.item_pedido_id = v_item.id;

    return query select v_pedido.id, v_item.id, v_item.cantidad, false;
    return;
  end if;

  insert into public.preparacion_items_pedido (
    pedido_id,
    item_pedido_id,
    cantidad_preparada,
    motivo_faltante,
    ...
    v_pedido.id,
    v_item.id,
    p_cantidad_preparada,
    v_motivo,
    auth.uid()
  )
  on conflict (item_pedido_id) do update
...
create or replace function public.marcar_pedido_preparando_administrativo(
  p_pedido_id uuid
)
returns uuid
language plpgsql
...
begin
  ...
  set estado = 'preparando',
      preparacion_estado = coalesce(preparacion_estado, 'pendiente')
  where id = v_pedido.id;
  return v_pedido.id;
end;
$$;

create or replace function public.marcar_pedido_listo_despacho_administrativo(
  p_pedido_id uuid
)
returns uuid
language plpgsql
...
begin
  ...
  if v_pedido.preparacion_estado = 'pendiente' then raise exception 'PREPARACION_PENDIENTE_DE_FINALIZACION'; end if;
