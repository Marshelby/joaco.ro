-- La cancelación por cliente conserva la historia operativa y financiera.
-- El cliente sólo puede cancelar su propio pedido mientras continúa recibido.

create function public.cancelar_pedido_cliente(
  p_pedido_id uuid,
...
begin
  ...
  where pedido_id = v_pedido.id and activa;

  update public.pedidos set estado = 'cancelado'
  where id = v_pedido.id
  returning * into v_pedido;

  update public.historial_estados_pedido
  set observacion = v_observacion
  where id = (
    select h.id from public.historial_estados_pedido h
    where h.pedido_id = v_pedido.id and h.estado_nuevo = 'cancelado'
