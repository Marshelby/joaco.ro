...
type PedidoConfirmacion = { numero_pedido: string; estado: EstadoPedidoAdmin; fecha_creacion: string; total: number | string; direccion_snapshot: string | null; comuna_snapshot: string | null; region_snapshot: string | null; destinatario_entrega_snapshot: string | null; zona_entrega_snapshot: string | null; latitud_entrega_snapshot: number | string | null; longitud_entrega_snapshot: number | string | null; items_pedido: ItemConfirmacion[] };
...
export default async function OrderConfirmationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase
    .from("pedidos")
    .select("numero_pedido,estado,fecha_creacion,total,direccion_snapshot,comuna_snapshot,region_snapshot,destinatario_entrega_snapshot,zona_entrega_snapshot,latitud_entrega_snapshot,longitud_entrega_snapshot,items_pedido(id,nombre_producto_snapshot,nombre_presentacion_snapshot,unidad_snapshot,cantidad,total_linea)")
    .eq("id", id)
    .maybeSingle();
