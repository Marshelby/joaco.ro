...
import { CATALOG_SORT_OPTIONS, getCatalogCategoryParam, getCatalogHref, type CatalogFilters, type CatalogSort } from "@/lib/catalog";
    ...
    <section className="space-y-5 rounded-xl border border-border bg-card p-4 sm:p-5" aria-label="Buscar y filtrar productos">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <form action="/catalogo" className="flex min-w-0 flex-1 gap-2" role="search">
          {filters.category ? <input type="hidden" name="categoria" value={getCatalogCategoryParam(filters.category)} /> : null}
      ...
      <nav className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Filtrar por categoría">
        ...
        {categories.map((category) => {
          ...
          return <Link key={category} href={getCatalogHref({ query: filters.query, category, sort: filters.sort })} aria-current={active ? "page" : undefined} className={`inline-flex min-h-10 shrink-0 items-center rounded-full border px-3 text-sm font-medium outline-none transition-colors focus-visible:ring-3 focus-visible:ring-ring/50 ${active ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background text-foreground hover:bg-muted"}`}>{category}</Link>;
        })}
      </nav>
      ...
      {hasActiveFilters ? <div className="flex flex-wrap items-center gap-2 text-sm" aria-label="Filtros activos">
        ...
        {filters.category ? <Link href={getCatalogHref({ query: filters.query, sort: filters.sort })} aria-label={`Quitar categoría ${filters.category}`} className="inline-flex min-h-9 items-center gap-1 rounded-full bg-muted px-3 font-medium text-foreground outline-none hover:bg-secondary focus-visible:ring-3 focus-visible:ring-ring/50">{filters.category} <X className="size-3.5" aria-hidden="true" /></Link> : null}
