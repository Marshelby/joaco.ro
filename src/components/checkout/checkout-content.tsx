...
import { PAYMENT_INTENT_OPTIONS, type MetodoPagoPrevisto } from "@/lib/payment-intent";
...
export type EstadoCuentaCheckout = "guest" | "admin" | "inactive_customer" | "account_error" | "active_customer";

export function CheckoutContent({ claveIdempotencia, cliente, direcciones, fechasEntrega, estadoCuenta }: { claveIdempotencia: string; cliente: ClienteCheckout | null; direcciones: readonly DireccionCheckout[]; fechasEntrega: readonly FechaEntregaDisponible[]; estadoCuenta: EstadoCuentaCheckout }) {
  ...
  const [metodosPagoPrevistos, setMetodosPagoPrevistos] = useState<MetodoPagoPrevisto[]>([]);
  ...
  const opcionesPago = [
    ...
    { value: "transferencia" as const, label: "Transferencia", icon: Landmark },
    { value: "tarjeta" as const, label: "Tarjeta", icon: CreditCard },
  ];

  function alternarMetodoPago(metodo: MetodoPagoPrevisto) {
    setMetodosPagoPrevistos((actuales) => PAYMENT_INTENT_OPTIONS.filter((opcion) => opcion === metodo ? !actuales.includes(opcion) : actuales.includes(opcion)));
  }

  if (!isHydrated) return <p className="text-sm text-muted-foreground">Cargando tu pedido…</p>;

  if (items.length === 0) {
    return <section className="rounded-2xl border border-dashed border-border bg-card p-8 text-center sm:p-12"><h1 className="text-2xl font-semibold tracking-tight text-foreground">No hay productos para continuar</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">Agrega productos al carrito antes de confirmar un pedido.</p><Button render={<PublicLink href={ROUTES.catalog} />} className="mt-6">Ver catálogo</Button></section>;
  }

  if (estadoCuenta !== "active_customer" || !cliente) {
    if (estadoCuenta === "guest") {
      return <FadeIn><section className="rounded-2xl border border-border bg-card p-8 text-center sm:p-12"><h1 className="text-2xl font-semibold tracking-tight text-foreground">Estás a un paso de completar tu pedido</h1><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">Inicia sesión o crea tu cuenta para continuar. Con Google puedes hacerlo en segundos.</p><div className="mx-auto mt-6 max-w-sm space-y-3"><GoogleLoginButton returnTo={ROUTES.checkout} /><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Button render={<PublicLink href={hrefConReturnTo("/iniciar-sesion?modo=iniciar", ROUTES.checkout)} />} variant="outline">Iniciar sesión</Button><Button render={<PublicLink href={hrefConReturnTo("/iniciar-sesion?modo=crear", ROUTES.checkout)} />}>Crear cuenta</Button></div></div></section></FadeIn>;
    }
    ...
    <form action={accion} className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start lg:gap-8">
      ...
      <section className="space-y-6">
        ...
        <fieldset className="rounded-xl border border-border bg-card p-4 sm:p-5">
          <legend className="text-lg font-semibold tracking-tight text-foreground">¿Cómo pagarás el día de la entrega?</legend>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">Puedes elegir más de una opción.</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            {opcionesPago.map(({ value, label, icon: Icon }) => {
              const seleccionado = metodosPagoPrevistos.includes(value);
              return <label key={value} className={`flex min-h-14 cursor-pointer items-center gap-3 rounded-lg border p-3 text-sm font-medium outline-none transition-colors focus-within:ring-3 focus-within:ring-ring/50 ${seleccionado ? "border-primary bg-primary/8 text-foreground" : "border-input bg-background text-foreground hover:bg-muted"}`}><input type="checkbox" name="metodosPagoPrevistos" value={value} checked={seleccionado} onChange={() => alternarMetodoPago(value)} className="size-4 accent-primary" /><Icon aria-hidden="true" className="size-5 text-muted-foreground" /><span>{label}</span></label>;
            })}
          </div>
        </fieldset>
      ...
      <aside className="rounded-2xl border border-border bg-card p-5 sm:p-6 lg:sticky lg:top-6"><h2 className="text-lg font-semibold text-foreground">Resumen del pedido</h2><ul className="mt-4 divide-y divide-border">{items.map((item) => <li key={`${item.productoId}:${item.presentacionId}`} className="py-3 first:pt-0"><div className="flex justify-between gap-3"><div className="min-w-0"><p className="text-sm font-medium text-foreground">{item.nombre}</p><p className="mt-1 text-xs text-muted-foreground">{item.presentacionNombre} · {formatCartQuantity(item, item.cantidad)}</p><p className="mt-1 text-xs text-muted-foreground">{formatCLP(item.precioFinalReferencia)} {isFractionalKgItem(item) ? "por kg" : "por presentación"}</p></div><p className="shrink-0 text-sm font-semibold text-foreground">{formatCLP(getCartLineSubtotal(item))}</p></div></li>)}</ul><dl className="mt-4 space-y-3 border-t border-border pt-4 text-sm"><div className="flex items-center justify-between gap-4"><dt className="text-muted-foreground">Subtotal</dt><dd className="font-medium text-foreground">{formatCLP(cotizacion.subtotal)}</dd></div><div className="flex items-center justify-between gap-4"><dt className="text-muted-foreground">Despacho</dt><dd className="font-medium text-foreground">{cotizacion.costoEntrega > 0 ? formatCLP(cotizacion.costoEntrega) : "Gratis"}</dd></div><div className="flex items-end justify-between gap-4 border-t border-border pt-4"><dt className="font-medium text-muted-foreground">Total estimado</dt><dd><strong className="text-2xl tracking-tight text-foreground">{formatCLP(cotizacion.totalEstimado)}</strong></dd></div></dl><p className="mt-3 text-xs leading-5 text-muted-foreground">El total definitivo se calcula con los precios vigentes al confirmar.</p><PendingButton type="submit" pending={pendiente} pendingLabel="Confirmando…" disabled={!claveIdempotencia || !carritoValido || !direccionClienteId || !fechaEntrega || fechasEntrega.length === 0 || metodosPagoPrevistos.length === 0} className="mt-5 w-full">Confirmar pedido</PendingButton><p aria-live="polite" className="mt-3 text-sm text-destructive">{estado.error ?? ""}</p></aside>
    </form>
  );
}
